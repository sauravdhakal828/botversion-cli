...
def _get_user_from_request(request, framework):
    ...
    try:
        if framework == "django":
            ...
            # Read session — contains workspaceId, projectId etc set at login
            ...
            # Read real DB user object via Flask-Login
            try:
            ...
            # Read Flask session — contains IDs set at login
            try:
                ...
                if g_user and isinstance(g_user, dict):
            ...
            # Step 1: Read request.state.user
            ...
            # Step 2: Read everything else on request.state
            ...
            # Step 3: Read JWT payload from Authorization header
            ...
            if token:
                try:
                    ...
                    payload = _json.loads(base64.b64decode(payload_part).decode("utf-8"))
                    for key, value in payload.items():
                        if key not in ("exp", "iat", "nbf", "jti", "iss", "aud", "type") and key not in result:
                            result[key] = value
                except Exception:
                    pass
